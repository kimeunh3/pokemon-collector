...
function StatsStatisticsPage() {
  ...
  useEffect(() => {
    ...
    Object.keys(newY.typesStatsSum).forEach(pokemonType => {
      const typemean = newY.typesStatsSum[pokemonType]/newY.typesCnt[pokemonType]
      newY.typesMeans[pokemonType] = Number(typemean.toFixed(1));
    })

    // 상위 30%, 하위 30% 속성 순위 차트에 들어갈 평균값
    const newStatsMean = Number(average(Object.values(newY.pokemonsStats)).toFixed(1));
    setStatsMean(newStatsMean);

    // 상위 10%, 하위 10% 포켓몬 순위 차트에 들어갈 평균값
    const newTypesStatsMean = Number(average(Object.values(newY.typesMeans)).toFixed(1));
    setTypesStatsMean(newTypesStatsMean);

    // 상위 30%, 하위 30% 속성 순위 차트에 입력할 데이터 생성
    const sortTypeMeans = Object.keys(newY.typesMeans).sort((a,b) => newY.typesMeans[b]-newY.typesMeans[a]);
    ...
    const sortPokemonsStats = Object.keys(newY.pokemonsStats).sort((a,b) => newY.pokemonsStats[b]-newY.pokemonsStats[a]);
