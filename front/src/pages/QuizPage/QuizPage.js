...
function QuizPage() {
	...
	return (
		<div style={{ paddingTop: '170px' }}>
            ...
            {!isMobile && isQuizEx1 && (
                <QuizComponent set1={setIsQuizEx1} set2={setIsQuizEx2} text={['포켓몬 퀴즈에 온 걸 환영한다!', <br/>, '여기엔 151마리의 포켓몬이 있단다.']} />
            )}
            {!isMobile && isQuizEx2 && (
                <QuizComponent set1={setIsQuizEx2} set2={setIsQuizEx3} text={['포켓몬의 실루엣을 보여줄테니', <br/>, '잘 보고 어떤 포켓몬인지 맞추면 된단다.']} />
            )}
            {!isMobile && isQuizEx3 && (
                <QuizComponent set1={setIsQuizEx3} set2={setIsQuizEx4} text={['정답을 맞추면 500포인트를 받을 수 있지.']} />
            )}
            {!isMobile && isQuizEx4 && (
                <QuizComponent set1={setIsQuizEx4} set2={setIsQuizEx5} text={['기회는 3번!', <br/>, '어려우면 힌트를 받거나 패스를 하렴.']} />
            )}
            {!isMobile && isQuizEx5 && (
            ...
            {isMobile && (
                <QuizComponent isMobile text='퀴즈는 pc에서!' />
            )}
		</div>
	);
}
